"""
make_figure.py — standalone combined nearest-neighbour figure.

Requires only numpy and matplotlib (+ scienceplots for the science version).
All image data is pre-cached.

Panel A (top):  Query 172 — HSC/Legacy rows with physics & instrument NNs.
Panel B (bottom): Lens 48   — physics NNs (row 2 of lens_neighbors_figure).
...
# Register Liberation Serif (metric-compatible Times New Roman substitute)
...
def lens_display_rank(r):
    """Subtract 3 for ranks 4-8 (skip trivial self-matches 1-3),
    subtract 4 for ranks 9+ (also skip rank 8, the excluded Legacy counterpart)."""
...
def make_figure(out_png, out_pdf, fontsize_offset=0):
    ...
    img_w, img_h = 2.2, 2.2
    fig = plt.figure(figsize=(n_cols * img_w, 3 * img_h + 0.9))

    # Rows: title | HSC | Legacy | gap | Lens
    gs = GridSpec(5, n_cols, figure=fig,
                  height_ratios=[0.18, 1, 1, 0.20, 1],
                  hspace=0.06, wspace=0.06,
                  top=0.97, bottom=0.02, left=0.01, right=0.99)

    axes_hsc  = [fig.add_subplot(gs[1, c]) for c in range(n_cols)]
    axes_leg  = [fig.add_subplot(gs[2, c]) for c in range(n_cols)]
    axes_lens = [fig.add_subplot(gs[4, c]) for c in range(n_cols_B)]
    for c in range(n_cols):
        fig.add_subplot(gs[3, c]).set_axis_off()
    for c in range(n_cols_B, n_cols):
        fig.add_subplot(gs[4, c]).set_axis_off()

    fs_label = 12 + fontsize_offset   # image overlay labels
    fs_title = 14 + fontsize_offset   # section titles
    fs_lens_query = 8 + fontsize_offset  # lens query ID label

    # Panel A
    show(axes_hsc[0], nn_rgb(q["query_hsc"]),   "HSC",    fontsize=fs_label)
    show(axes_leg[0], nn_rgb(q["query_legacy"]), "Legacy", fontsize=fs_label)
    ...
    for i, (img, src, rank) in enumerate(zip(
            q["leg_phys_imgs"], q["leg_phys_srcs"], q["leg_phys_ranks"])):
        color = "indianred" if rank == 1 else ("#B8860B" if src == "hsc" else "black")
        show(axes_leg[1+i], nn_rgb(img), f"NN#{rank} {fmt(src)}", color, fontsize=fs_label)

    for i, (img, src, rank) in enumerate(zip(
            q["leg_inst_imgs"], q["leg_inst_srcs"], q["leg_inst_ranks"])):
        color = "#B8860B" if src == "hsc" else "black"
        show(axes_leg[1+n_phys+i], nn_rgb(img), f"NN#{rank} {fmt(src)}", color, fontsize=fs_label)
    ...
    # Dashed separators — Panel A
    y_top = axes_hsc[0].get_position().y1 + 0.005
    y_bot = axes_leg[0].get_position().y0 - 0.005
    ...
    # Horizontal rule — close to bottom of top panel
    gap_y0 = axes_lens[0].get_position().y1
    gap_y1 = axes_leg[0].get_position().y0
    rule_y = gap_y1 - 0.2 * (gap_y1 - gap_y0)
...
if __name__ == "__main__":
    base = _here / "nearest_neighbors_figure"

    # ── Version 1: Liberation Serif (Times New Roman substitute) ─────────────
