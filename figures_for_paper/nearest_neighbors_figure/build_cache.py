"""
build_cache.py — regenerate _cache/query_172.npz from scratch.

Requires the full environment: torch, h5py, sklearn, galaxy_images.

Inputs:
  neighbor_latents_103k.h5   — pre-computed gallery embeddings
  neighbours_v2.h5           — raw image store
  checkpoints/base/snapshot.ckpt  — not loaded here; embeddings already done

Run from repo root:
    OPENBLAS_NUM_THREADS=4 OMP_NUM_THREADS=4 \
    python figures_for_paper/nearest_neighbors_figure/build_cache.py
"""
import sys
...
OUT          = Path(__file__).resolve().parent / "_cache" / "query_172.npz"
