...
notify "🔍 [job ${SLURM_JOB_ID}] Lens ${LENS_INDEX} combined search (overlap ~103k pairs | ours vs AION | L2 + Cosine)..."
...
FIG="${OUT_DIR}/lens_$(printf '%03d' $LENS_INDEX)_hsc_aion_combined.png"
curl -s -X POST "$DISCORD" \
    -F "payload_json={\"content\": \"**Lens ${LENS_INDEX} — HSC+Legacy overlap (~103k pairs) | ours vs AION | L2 + Cosine**\"}" \
    -F "file=@${FIG}" > /dev/null
