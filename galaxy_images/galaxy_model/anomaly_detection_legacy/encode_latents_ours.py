"""
Legacy-primary variant of anomaly_detection/encode_latents_ours.py.

Encodes all source_type=0 (MMU) examples with encoder_1 (physics latent) of the
trained dual-encoder model, but using Legacy images as the primary input.

Dataset names inside the output file are kept identical to the HSC pipeline so
that fit_and_score.py works unchanged. Semantics:
  hsc_flat        -> flatten of encoder_1(leg_imgs)
  hsc_mean        -> mean-pool of encoder_1(leg_imgs)
  hsc_legacy_mean -> mean of [encoder_1(leg) ⊕ encoder_1(hsc)] tokens
The file attribute primary_survey="legacy" disambiguates the file.

Run from galaxy_model/:
  python anomaly_detection_legacy/encode_latents_ours.py \
    --checkpoint outputs/neighbors_all_attn/2026-04-05/checkpoints/best-epoch=228-step=87000.ckpt \
    --suffix ours_legacy_100k \
...
def encode(checkpoint_path, suffix, batch_size, n_max, device_str):
    ...
    with torch.no_grad():
        for hsc_imgs, leg_imgs, raw_indices in tqdm(loader, desc="Encoding"):
            ...
            enc1_leg = model.encoder_1(leg_imgs)   # primary  [B, n_tokens, cross_dim]
            enc1_hsc = model.encoder_1(hsc_imgs)   # auxiliary

            hsc_flat = enc1_leg.flatten(start_dim=1).cpu().numpy().astype(np.float32)
            hsc_mean = enc1_leg.mean(dim=1).cpu().numpy().astype(np.float32)
            hsc_legacy_mean = torch.cat([enc1_leg, enc1_hsc], dim=1).mean(dim=1).cpu().numpy().astype(np.float32)
    ...
    print(f"  hsc_flat:  {hsc_flat.shape}  (n_tokens={enc1_leg.shape[1]}, cross_dim={cross_dim})")
    ...
    tmp_fd, tmp_path = tempfile.mkstemp(suffix=".h5", prefix="anomaly_latents_ours_legacy_", dir=tempfile.gettempdir())
    os.close(tmp_fd)
    try:
        with h5py.File(tmp_path, "w") as f:
            ...
            f.attrs["n_tokens"] = int(enc1_leg.shape[1])
...
def main():
    ...
    parser.add_argument("--suffix", default="ours_legacy_100k")
