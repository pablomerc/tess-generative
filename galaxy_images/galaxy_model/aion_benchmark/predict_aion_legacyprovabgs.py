"""
Downstream evaluation for AION on Legacy ProvaBGS: train an MLP to predict galaxy/physics
parameters from mean AION embeddings (from extract_latents_aion_legacyprovabgs.py).

Uses same target columns as predict_legacy_provabgs / prepare_all (physics_provabgs).
Default dataset: downstream_aion_legacy_provabgs.h5.

Run:
  python aion_benchmark/predict_aion_legacyprovabgs.py
  python aion_benchmark/predict_aion_legacyprovabgs.py --dataset path/to/downstream_aion_legacy_provabgs.h5 --targets desi_Z LOG_MSTAR
"""
import argparse
import csv
...
# Default path (from extract_latents_aion_legacyprovabgs.py)
DATASET_PATH = _here / "downstream_aion_legacy_provabgs.h5"
R2_OUT_DIR = _here / "r2_comparisons_aion_legacyprovabgs"

# Legacy ProvaBGS targets (same as prepare_all / predict_legacy_provabgs)
physics_provabgs = ["desi_Z", "LOG_MSTAR", "TAGE_MW", "LOG_Z_MW", "sSFR", "hsc_g_extendedness_value", "DEC"]
DEFAULT_TARGETS = physics_provabgs

# Embedding keys (same as predict_aion)
EMBEDDING_KEYS = [
...
def load_h5_aion_legacyprovabgs(path, verbose=True):
    """
    Load mean AION embeddings and numeric labels from downstream_aion_legacy_provabgs.h5.
    Returns embeddings_dict (name -> (N, dim)), metadata (N, num_targets), param_names.
    Only keeps numeric label columns (no derived HSC PSF columns).
    """
...
def train_and_eval(train_loader, val_loader, param_names, emb_dim, out_dim, use_gpu, precision):
    ...
    ckpt = ModelCheckpoint(
        ...
        mode="min",
        save_top_k=1,
        filename="aion_legacyprovabgs-{epoch:02d}-{val_loss:.4f}",
    )
    trainer = Trainer(
        max_epochs=100,
...
def save_plot(results_by_experiment, param_names, filepath):
    ...
    ax.set_ylabel("R²")
    ax.set_title("AION mean embeddings → Legacy ProvaBGS parameters (MLP)")
    ax.set_xticks(x)
...
def main():
    p = argparse.ArgumentParser(
        description="Train MLP to predict Legacy ProvaBGS parameters from mean AION embeddings"
    )
    p.add_argument(
        "--dataset",
        type=str,
        default=str(DATASET_PATH),
        help="Path to downstream_aion_legacy_provabgs.h5",
    )
    p.add_argument(
        "--out-dir",
        type=str,
    ...
    embeddings_dict, meta, param_names = load_h5_aion_legacyprovabgs(str(dataset_path))
    ...
    print("\n" + "=" * (32 + col_w * len(names)))
    print("R² — AION mean embeddings → Legacy ProvaBGS parameters")
