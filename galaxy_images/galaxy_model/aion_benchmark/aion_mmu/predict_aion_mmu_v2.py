"""
Downstream evaluation for AION on MMU (v2): train an MLP to predict galaxy/instrument
parameters from mean AION embeddings produced by aion_mmu_match_and_extract.py.

Uses the matched H5 (downstream_aion_mmu_matched.h5), where IDs are aligned by HSC object ID
between metadata and neighbours. Same target columns as MMU: physics_mmu + instrument_mmu_legacy + instrument_mmu_hsc.

Run:
  python aion_benchmark/predict_aion_mmu_v2.py
  python aion_benchmark/predict_aion_mmu_v2.py --dataset path/to/downstream_aion_mmu_matched.h5 --targets SHAPE_E1 SHAPE_R EBV
"""
import argparse
import csv
...
# Default path: output of aion_mmu_match_and_extract.py
DATASET_PATH = _here / "downstream_aion_mmu_matched.h5"
R2_OUT_DIR = _here / "r2_comparisons_aion_mmu_v2"

# MMU targets (same as aion_mmu_match_and_extract / prepare_all)
physics_mmu = ["SHAPE_E1", "SHAPE_E2", "SHAPE_R"]
instrument_mmu_legacy = ["EBV"]
instrument_mmu_hsc = ["a_g", "a_i", "a_r", "a_y", "a_z"]
DEFAULT_TARGETS = physics_mmu + instrument_mmu_legacy + instrument_mmu_hsc

# Embedding keys (same structure as aion_mmu_match_and_extract output)
EMBEDDING_KEYS = [
...
def load_h5_aion_mmu(path, verbose=True):
    """
    Load mean AION embeddings and numeric labels from downstream_aion_mmu_matched.h5
    (or any H5 with same layout: embeddings_mean, embeddings_mean_legacy, embeddings_mean_hsc, labels/<col>).
...
def train_and_eval(train_loader, val_loader, param_names, emb_dim, out_dim, use_gpu, precision):
    ...
    ckpt = ModelCheckpoint(
        ...
        mode="min",
        save_top_k=1,
        filename="aion_mmu_v2-{epoch:02d}-{val_loss:.4f}",
    )
    trainer = Trainer(
        max_epochs=100,
...
def save_plot(results_by_experiment, param_names, filepath):
    ...
    offsets = (
        [-(width / 2), (width / 2)]
        if n_names == 2
        else np.linspace(-width * (n_names - 1) / 2, width * (n_names - 1) / 2, n_names)
    )
    if n_names != 2:
        offsets = list(offsets)
    colors = ["#2E86AB", "#E07A5F", "#81B29A"][:n_names]
    ...
    ax.set_ylabel("R²")
    ax.set_title("AION mean embeddings → MMU parameters (MLP) [v2 matched IDs]")
    ax.set_xticks(x)
...
def main():
    p = argparse.ArgumentParser(
        description="Train MLP to predict MMU parameters from mean AION embeddings (v2: matched H5)"
    )
    p.add_argument(
        "--dataset",
        type=str,
        default=str(DATASET_PATH),
        help="Path to downstream_aion_mmu_matched.h5",
    )
    p.add_argument(
        "--out-dir",
        type=str,
    ...
    p.add_argument(
        ...
        nargs="*",
        default=None,
        help="Target columns (default: MMU physics + instrument); empty = all numeric labels",
    )
    ...
    target_columns = (
        ...
        else (args.targets if args.targets is not None else DEFAULT_TARGETS)
    )
    ...
    embeddings_dict, meta, param_names = load_h5_aion_mmu(str(dataset_path))
    ...
    for name in embeddings_dict:
        embeddings_dict[name] = np.nan_to_num(
            embeddings_dict[name][finite], nan=0.0, posinf=0.0, neginf=0.0
        )
    meta = meta[finite]
    n = len(meta)
    ...
    print("\n" + "=" * (32 + col_w * len(names)))
    print("R² — AION mean embeddings → MMU parameters (v2 matched)")
    print("=" * (32 + col_w * len(names)))
    header = f"{'Target':<32} " + " ".join(
        f"{EMBEDDING_LABELS.get(n, n):>{col_w}}" for n in names
    )
    print(header)
    print("-" * (32 + col_w * len(names)))
    for t in param_names:
        r2s = [
            next((r["r2"] for r in results_by_experiment[n] if r["target"] == t), np.nan)
            for n in names
        ]
