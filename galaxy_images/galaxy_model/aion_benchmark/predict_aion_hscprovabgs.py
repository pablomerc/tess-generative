"""
Downstream evaluation for AION on HSC ProvaBGS: train an MLP to predict galaxy/physics
parameters from mean AION embeddings (from extract_latents_aion_hscprovabgs.py).

Uses same target columns as predict_hsc_provabgs (physics_columns).
Default dataset: downstream_aion_hsc_provabgs.h5.

Run:
  python aion_benchmark/predict_aion_hscprovabgs.py
  python aion_benchmark/predict_aion_hscprovabgs.py --dataset path/to/downstream_aion_hsc_provabgs.h5 --targets desi_Z LOG_MSTAR
"""
import argparse
import csv
...
# Default path (from extract_latents_aion_hscprovabgs.py)
DATASET_PATH = _here / "downstream_aion_hsc_provabgs.h5"
R2_OUT_DIR = _here / "r2_comparisons_aion_hscprovabgs"

# HSC ProvaBGS targets (same as predict_hsc_provabgs physics_columns)
physics_hsc_provabgs = ["sSFR", "LOG_Z_MW", "TAGE_MW", "LOG_MSTAR", "desi_Z"]
DEFAULT_TARGETS = physics_hsc_provabgs
...
def load_h5_aion_hscprovabgs(path, verbose=True):
    """
    Load mean AION embeddings and numeric labels from downstream_aion_hsc_provabgs.h5.
...
def train_and_eval(train_loader, val_loader, param_names, emb_dim, out_dim, use_gpu, precision):
    ...
    ckpt = ModelCheckpoint(
        ...
        mode="min",
        save_top_k=1,
        filename="aion_hscprovabgs-{epoch:02d}-{val_loss:.4f}",
    )
    trainer = Trainer(
        max_epochs=100,
...
def save_plot(results_by_experiment, param_names, filepath):
    ...
    ax.set_ylabel("R²")
    ax.set_title("AION mean embeddings → HSC ProvaBGS parameters (MLP)")
    ax.set_xticks(x)
...
def main():
    p = argparse.ArgumentParser(
        description="Train MLP to predict HSC ProvaBGS parameters from mean AION embeddings"
    )
    p.add_argument(
        "--dataset",
        type=str,
        default=str(DATASET_PATH),
        help="Path to downstream_aion_hsc_provabgs.h5",
    )
    p.add_argument(
        "--out-dir",
        type=str,
    ...
    p.add_argument(
        ...
        nargs="*",
        default=None,
        help="Target columns (default: physics_hsc_provabgs); empty = all numeric labels",
    )
    ...
    embeddings_dict, meta, param_names = load_h5_aion_hscprovabgs(str(dataset_path))
    ...
    print("\n" + "=" * (32 + col_w * len(names)))
    print("R² — AION mean embeddings → HSC ProvaBGS parameters")
