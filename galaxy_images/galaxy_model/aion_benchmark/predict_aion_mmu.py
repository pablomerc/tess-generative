"""
Downstream evaluation for AION on MMU: train an MLP to predict galaxy/instrument
parameters from mean AION embeddings (from extract_latents_aion_mmu.py).

Uses same target columns as prepare_all MMU (physics_mmu + instrument_mmu_legacy + instrument_mmu_hsc).
Default dataset: downstream_aion_mmu.h5.

Run:
  python aion_benchmark/predict_aion_mmu.py
  python aion_benchmark/predict_aion_mmu.py --dataset path/to/downstream_aion_mmu.h5 --targets SHAPE_E1 SHAPE_R EBV
"""
import argparse
import csv
...
# Default path (from extract_latents_aion_mmu.py)
DATASET_PATH = _here / "downstream_aion_mmu.h5"
R2_OUT_DIR = _here / "r2_comparisons_aion_mmu"

# MMU targets (same as extract_latents_aion_mmu / prepare_all)
...
# Embedding keys (same structure as extract_latents_aion_mmu output)
EMBEDDING_KEYS = [
...
def load_h5_aion_mmu(path, verbose=True):
    """
    Load mean AION embeddings and numeric labels from downstream_aion_mmu.h5.
...
def train_and_eval(train_loader, val_loader, param_names, emb_dim, out_dim, use_gpu, precision):
    ...
    ckpt = ModelCheckpoint(
        ...
        mode="min",
        save_top_k=1,
        filename="aion_mmu-{epoch:02d}-{val_loss:.4f}",
    )
    trainer = Trainer(
        max_epochs=100,
...
def save_plot(results_by_experiment, param_names, filepath):
    ...
    ax.set_ylabel("R²")
    ax.set_title("AION mean embeddings → MMU parameters (MLP)")
    ax.set_xticks(x)
...
def main():
    p = argparse.ArgumentParser(
        description="Train MLP to predict MMU parameters from mean AION embeddings"
    )
    p.add_argument(
        "--dataset",
        type=str,
        default=str(DATASET_PATH),
        help="Path to downstream_aion_mmu.h5",
    )
    p.add_argument(
        "--out-dir",
        type=str,
    ...
    print("\n" + "=" * (32 + col_w * len(names)))
    print("R² — AION mean embeddings → MMU parameters")
