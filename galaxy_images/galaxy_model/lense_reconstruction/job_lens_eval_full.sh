...
OUTDIR="lense_reconstruction/outputs/lens_eval_hier_global_ins_full"
...
notify "🔭 [job $SLURM_JOB_ID] lens-eval-full starting — hier-global-ins step=201k, all 74 lenses..."
...
if [ $? -ne 0 ]; then
    notify "❌ [job $SLURM_JOB_ID] lens-eval-full FAILED — check logs"
    exit 1
fi

notify_file "🌌 [job $SLURM_JOB_ID] Lens eval (74 lenses) — normalized: SameGal | SameIns | Target | 5 Samples | Mean" \
    "${OUTDIR}/lens_eval_normalized.png"
notify_file "🌌 [job $SLURM_JOB_ID] Lens eval (74 lenses) — row-scaled (from target vmin/vmax)" \
    "${OUTDIR}/lens_eval_row_scaled.png"
notify "✅ [job $SLURM_JOB_ID] lens-eval-full DONE"
