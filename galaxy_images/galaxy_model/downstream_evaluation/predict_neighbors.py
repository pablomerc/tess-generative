"""
Downstream evaluation for Neighbors: predict galaxy parameters from embeddings.
Uses H5 from prepare_neighbors.py (6 embedding types + labels from dataset metadata).

Run a single experiment per run: legacy, legacy_hsc, or hsc.
  python predict_neighbors.py --experiment legacy
  python predict_neighbors.py --experiment legacy_hsc
  python predict_neighbors.py --experiment hsc

Run on a list of .h5 datasets (all experiments per dataset); saves plot + CSV per (dataset, experiment).
  python predict_neighbors.py --datasets a.h5 b.h5 c.h5
  python predict_neighbors.py   # uses default 3 H5s from prepare_neighbors
"""
import argparse
import csv
...
# Default paths (Neighbors downstream H5s from prepare_neighbors.py)
DATASET_PATH = _here / "downstream_neighbors_zdim16_geom_neighbors.h5"
DATASET_PATH_UNTRAINED = _here / "downstream_neighbors_untrained.h5"

DEFAULT_DATASET_LIST = [
    _here / "downstream_neighbors_zdim16_geom_neighbors.h5",
    _here / "downstream_neighbors_zdim16_nogeom_neighbors.h5",
    _here / "downstream_neighbors_zdim16_geom_old_dataloader.h5",
]
R2_COMPARISONS_DIR = _here / "r2_comparisons_neighbors"
...
# Default target columns (must exist in H5 labels from prepare_neighbors.py)
Legacy_columns = [
    "legacy_PSFSIZE_G",
    "legacy_PSFSIZE_I",
    "legacy_PSFSIZE_R",
...
# Derived from hsc_*_sdssshape_psf_shape11/22 via compute_hsc_psf_seeing (added in load_h5 when shape cols exist)
...
def load_h5(path, key1, key2, verbose=True):
    """
    Load two embedding arrays and numeric labels from Neighbors H5.
    Returns emb1, emb2, metadata (N, num_targets), param_names.
    1D arrays (n,) are kept as-is; 2D arrays (n, k) are split into k columns (col_0 .. col_{k-1}).
    Others are dropped (and reported if verbose).
    """
    with h5py.File(path, "r") as f:
        ...
        for col in label_columns:
            ...
            if arr.ndim == 1:
                ...
                # Split (n, k) into k separate 1D columns: col_0, col_1, ...
        ...
        if verbose and dropped:
            ...
            print(f"  Kept {len(param_names)} label columns (all shape ({n},)).")
        if not meta_list:
            raise ValueError("No numeric label columns found in H5 (or all dropped due to shape mismatch).")
        # Add derived HSC PSF FWHM (arcsec) for each band where shape11/22 exist
        for band in ("g", "i", "r", "z"):
...
def save_plot(results, param_names, experiment, filepath, dataset_stem=""):
    ...
    ax.set_ylabel("R²")
    title = f"Neighbors — {dataset_stem} — {experiment}" if dataset_stem else f"Neighbors — {experiment}"
...
def main():
    p = argparse.ArgumentParser(description="Neighbors downstream prediction (single or batch over datasets)")
    ...
    p.add_argument("--datasets", type=str, nargs="*", default=None,
                   help="List of .h5 paths; run all experiments on each. Default: 3 H5s from prepare_neighbors.")
    p.add_argument("--single", action="store_true",
                   help="Single-run mode: use --dataset and --experiment only.")
    ...
    p.add_argument("--out-dir", type=str, default=str(R2_COMPARISONS_DIR),
                   help="Output directory for plots and CSVs (default: r2_comparisons_neighbors)")
    p.add_argument("--targets", type=str, nargs="*", default=None,
                   help="Target columns (default: Legacy_columns + HSC_columns); empty = all numeric labels in H5")
    ...
    if not args.single:
        ...
        untrained_path = Path(args.dataset_untrained)
        if not untrained_path.exists():
            print(f"Untrained dataset not found: {untrained_path}")
            return
            ...
            for experiment in EXPERIMENTS:
                ...
                try:
                    results, param_names = run_one_experiment(
                        dataset_path, experiment, untrained_path,
    ...
    results, param_names = run_one_experiment(
        Path(args.dataset), args.experiment, Path(args.dataset_untrained),
        target_columns, args.seed, use_gpu, precision,
    )
