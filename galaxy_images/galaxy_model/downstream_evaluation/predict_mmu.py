"""
Downstream evaluation for MMU: predict galaxy parameters from embeddings.
Uses H5 from prepare_mmu.py (6 embedding types + labels from metadata H5).

Run a single experiment per run: legacy, legacy_hsc, or hsc.
  python predict_mmu.py --experiment legacy
  python predict_mmu.py --experiment legacy_hsc
  python predict_mmu.py --experiment hsc

Run on a list of .h5 datasets (all experiments per dataset); saves plot + CSV per (dataset, experiment).
  python predict_mmu.py --datasets a.h5 b.h5 c.h5
  python predict_mmu.py   # uses default 3 H5s from prepare_mmu
"""
import argparse
import csv
...
# Default paths (MMU downstream H5s)
DATASET_PATH = _here / "downstream_mmu_zdim16_geom_neighbors.h5"
DATASET_PATH_UNTRAINED = _here / "downstream_mmu_untrained.h5"

# Default list of trained H5s from prepare_mmu.py (3 model configs)
DEFAULT_DATASET_LIST = [
    _here / "downstream_mmu_zdim16_geom_neighbors.h5",
    _here / "downstream_mmu_zdim16_nogeom_neighbors.h5",
    _here / "downstream_mmu_zdim16_geom_old_dataloader.h5",
]
R2_COMPARISONS_DIR = _here / "r2_comparisons_mmu"

# One experiment per run: which embedding pair to use (same keys as prepare_mmu EMBEDDING_NAMES)
EXPERIMENTS = {
...
# Default target columns to predict (must exist in H5 labels from prepare_mmu.py).
# Edit this list to add/remove targets; use --targets on the CLI to override.

DEFAULT_TARGETS = ['EBV', 'FLUX_G', 'FLUX_I', 'FLUX_R',
    'FLUX_W1', 'FLUX_W2', 'FLUX_W3', 'FLUX_W4', 'FLUX_Z',
    'SHAPE_E1', 'SHAPE_E2', 'SHAPE_R',
    'a_g', 'a_i', 'a_r', 'a_y', 'a_z',
    'g_cmodel_mag', 'g_cmodel_magerr', 'g_sdssshape_psf_shape11',
    'g_sdssshape_psf_shape12', 'g_sdssshape_psf_shape22',
    ...
    'y_cmodel_mag', 'y_cmodel_magerr', 'y_extendedness_value', 'z_cmodel_mag',
    'z_sdssshape_psf_shape11', 'z_sdssshape_psf_shape12', 'z_sdssshape_psf_shape22']

def load_h5(path, key1, key2):
    """
    Load two embedding arrays and numeric labels from MMU H5.
...
def save_plot(results, param_names, experiment, filepath, dataset_stem=""):
    ...
    ax.set_ylabel("R²")
    title = f"MMU — {dataset_stem} — {experiment}" if dataset_stem else f"MMU — {experiment}"
...
def main():
    p = argparse.ArgumentParser(description="MMU downstream prediction (single or batch over datasets)")
    ...
    p.add_argument("--datasets", type=str, nargs="*", default=None,
                   help="List of .h5 paths; run all experiments on each. Default: run on the 3 H5s from prepare_mmu. Pass --dataset and --experiment for single-run instead.")
    ...
    p.add_argument("--out-dir", type=str, default=str(R2_COMPARISONS_DIR),
                   help="Output directory for plots and CSVs (default: r2_comparisons_mmu)")
    p.add_argument("--targets", type=str, nargs="*", default=None,
                   help="Target columns (default: DEFAULT_TARGETS in this file); empty = all numeric labels in H5")
