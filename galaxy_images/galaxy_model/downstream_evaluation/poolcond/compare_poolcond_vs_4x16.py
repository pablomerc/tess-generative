"""
Compare Ours poolcond (1x16) vs Ours 4x16 vs AION on downstream R².

Reads:
  - Poolcond (1x16): poolcond/predict_all_ours_poolcond.csv
...
Produces:
  - compare_poolcond_vs_4x16.png
  - compare_poolcond_vs_4x16_merged.csv

Usage:
  python compare_poolcond_vs_4x16.py
  python compare_poolcond_vs_4x16.py --no-aion
  python compare_poolcond_vs_4x16.py --no-scienceplots
"""
...
POOLCOND_CSV = _here / "predict_all_ours_poolcond.csv"
FINAL_TABLE_CSV = _final / "predict_all_zdim16_nogeom_neighbors_table.csv"
...
OUT_CSV = _here / "compare_poolcond_vs_4x16_merged.csv"
OUT_PNG = _here / "compare_poolcond_vs_4x16.png"
...
# Physics 4x16, Instrument 4x16, Physics pooled 1x16, Instrument pooled 1x16, AION, Untrained
BAR_COLORS = ["#2E86AB", "#1A5276", "#CC546D", "#922B21", "#6CA75D", "#9473C6"]
...
def load_poolcond():
    ...
    return agg.rename(columns={
        "r2_physics": "Physics_pooled",
        "r2_instrument": "Instrument_pooled",
        "r2_untrained": "Untrained",
    })


def load_4x16():
    # _table.csv is already averaged across tasks; columns:
    # group, target, Physics latents, Instrument latents, Untrained, Random
    df = pd.read_csv(FINAL_TABLE_CSV)
    df = df[~df["target"].isin(EXCLUDE_TARGETS)]
    return df[["target", "Physics latents", "Instrument latents"]].rename(columns={
        "Physics latents": "Physics_4x16",
        "Instrument latents": "Instrument_4x16",
    })
...
def average_by_patterns(df):
    numeric_cols = [c for c in ("Physics_4x16", "Instrument_4x16",
                                "Physics_pooled", "Instrument_pooled",
...
def make_plot(merged, include_aion):
    ...
    merged["_sort"] = np.where(
        merged["group_rank"] == 0,
        -merged["Physics_4x16"].fillna(-1),
        -merged["Instrument_4x16"].fillna(-1),
    )
    ...
    has_aion = include_aion and "AION" in merged.columns and merged["AION"].notna().any()

    series = [
        ("Physics latents 4x16",           merged["Physics_4x16"].values,      BAR_COLORS[0], False),
        ("Physics latents pooled 1x16",    merged["Physics_pooled"].values,    BAR_COLORS[1], False),
        ("Instrument latents 4x16",        merged["Instrument_4x16"].values,   BAR_COLORS[2], False),
        ("Instrument latents pooled 1x16", merged["Instrument_pooled"].values, BAR_COLORS[3], False),
        ("Untrained ResNet-18",            merged["Untrained"].values,         None,           True),
    ]
    if has_aion:
        aion_vals = merged.get("AION", pd.Series(dtype=float)).reindex(merged.index).values
        series.insert(4, ("AION-1", aion_vals, BAR_COLORS[4], False))

    n_series = len(series)
    w = 0.14 if n_series >= 6 else 0.16
    offsets = np.linspace(-(n_series - 1) / 2, (n_series - 1) / 2, n_series) * w

    fig_w = max(12, cur_x * 0.65)
...
def main():
    ...
    p.add_argument("--no-aion", action="store_true", help="Exclude AION series")
    ...
    # Outer join so targets present in either model are included
    merged = poolcond.merge(four_x16, on="target", how="outer")
    if aion is not None and not args.no_aion:
    ...
    make_plot(merged, not args.no_aion)
