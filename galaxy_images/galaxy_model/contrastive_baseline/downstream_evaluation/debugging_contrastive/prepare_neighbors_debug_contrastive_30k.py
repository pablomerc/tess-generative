...
This variant samples 8k examples uniformly without replacement from the
30k-index window [70400, 100400).
"""
...
POOL_START_INDEX = 70400
POOL_END_INDEX = 100400
SAMPLED_COUNT = 8000
NEIGHBORS_SEED = 42
...
def save_downstream_h5_all_variants(path, real_dict, untrained_dict, random_dict, labels_n, label_columns, source_indices):
    path = Path(path)
    ...
    with h5py.File(path, "w") as f:
        ...
        f.attrs["sample_source"] = f"random_sample_{SAMPLED_COUNT}_from_{POOL_START_INDEX}_{POOL_END_INDEX}"
    print(f"Saved: {path}")
...
def _sample_indices(seed):
    pool = np.arange(POOL_START_INDEX, POOL_END_INDEX, dtype=np.int64)
    if len(pool) < SAMPLED_COUNT:
        raise ValueError(f"Pool only has {len(pool)} entries, cannot sample {SAMPLED_COUNT}.")
    rng = np.random.default_rng(seed)
    sampled = np.sort(rng.choice(pool, size=SAMPLED_COUNT, replace=False))
    return sampled


def prepare_neighbors(checkpoint_path, output_dir, batch_size, seed, suffix):
    print(f"[Neighbors Debug 30k] Loading neighbors dataset from: {NEIGHBORS_HDF5}")
    full_dataset = NeighborsSimpleDataset(hdf5_path=NEIGHBORS_HDF5)
    if len(full_dataset) < POOL_END_INDEX:
        raise ValueError(
            f"Neighbors dataset has {len(full_dataset)} entries, but this run requires at least {POOL_END_INDEX}."
        )

    sampled_indices = _sample_indices(seed)
    dataset = Subset(full_dataset, sampled_indices.tolist())
    print(
        "[Neighbors Debug 30k] Sampling configuration: "
        f"pool=[{POOL_START_INDEX}, {POOL_END_INDEX}), pool_size={POOL_END_INDEX - POOL_START_INDEX}, "
        f"sampled_count={SAMPLED_COUNT}, seed={seed}"
    )
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    def _emb_fn(model, stage_label):
        ...
        if meta_cache["labels_n"] is None:
            ...
            print(f"[Neighbors Debug 30k] Collected {len(label_columns)} numeric label columns.")
...
def main():
    p = argparse.ArgumentParser(description="Prepare neighbors embeddings from a random 8k sample of the 30k debug window.")
    p.add_argument("--checkpoint", required=True, help="Path to contrastive baseline checkpoint (.ckpt)")
    p.add_argument("--suffix", default=None, help="Output suffix. Default: checkpoint stem + _30k")
    p.add_argument("--output-dir", type=Path, default=_here / "outputs" / "neighbors_debug_30k",
                   help="Output directory for H5 files")
    p.add_argument("--batch-size", type=int, default=256)
    p.add_argument("--seed", type=int, default=42)
    args = p.parse_args()

    suffix = args.suffix or f"{Path(args.checkpoint).stem}_30k"
    output_dir = args.output_dir
    output_dir.mkdir(parents=True, exist_ok=True)

    print("[Neighbors Debug 30k]")
