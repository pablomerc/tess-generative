"""
Predict neighbors-only downstream objectives from OURS neighbors H5 (trained-only).

Consumes Stage-1 file:
  - downstream_neighbors_ours_{suffix}.h5

Runs three latent configurations:
  - hsc_only
  - legacy_only
  - hsc_legacy_combined
"""
...
LATENT_CONFIGS = [
    ("hsc_only", "hsc_encoder1", "hsc_encoder2"),
    ("legacy_only", "legacy_encoder1", "legacy_encoder2"),
    ("hsc_legacy_combined", "hsc_legacy_encoder1", "hsc_legacy_encoder2"),
]
...
def load_h5(path, key1, key2, add_neighbors_derived=False):
    with h5py.File(path, "r") as f:
        ...
        if key1 not in f or key2 not in f:
            raise ValueError(f"Missing {key1} or {key2} in {path}")
    ...
    if emb1.shape[0] != emb2.shape[0] or emb1.shape[0] != metadata.shape[0]:
        raise ValueError("Length mismatch between embeddings and labels")
...
def run_task(task_name, target_list, emb1, emb2, metadata, param_names, seed, use_gpu):
    ...
    task_param_names = [param_names[i] for i in indices]
    ...
    out = {
        "real_physics": train_and_eval_one(emb, meta, task_param_names, tr_idx, va_idx, use_embedding=1, use_gpu=use_gpu),
        "real_instrument": train_and_eval_one(emb, meta, task_param_names, tr_idx, va_idx, use_embedding=2, use_gpu=use_gpu),
        "mean": evaluate_mean_baseline(meta, task_param_names, tr_idx, va_idx),
    }
    return out, task_param_names
...
def plot_objective(df_obj, out_path, latent_label):
    ...
    ax.bar(x - width / 2, np.nan_to_num(vals_phys, nan=0.0), width, label="Physics latents", color="#8AC3EE")
    ax.bar(x + width / 2, np.nan_to_num(vals_inst, nan=0.0), width, label="Instrument latents", color="#E5254E")

    labels = [f"{t}\n({task})" for t, task in zip(df_plot["target"], df_plot["task"])]
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=45, ha="right", fontsize=9)
    ax.set_ylabel("R²")
    ax.set_title(f"Downstream Objective: neighbors ({latent_label})")
...
def main():
    p = argparse.ArgumentParser(description="Predict neighbors-only downstream objectives for OURS model (trained only).")
    p.add_argument("--suffix", required=True, help="Suffix used in prepare stage")
    p.add_argument("--output-dir", type=Path, default=Path(__file__).resolve().parent / "outputs",
                   help="Directory containing downstream_neighbors_ours_*.h5 and where outputs are written")
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--no-gpu", action="store_true")
    args = p.parse_args()

    output_dir = args.output_dir
    h5_path = output_dir / f"downstream_neighbors_ours_{args.suffix}.h5"
    if not h5_path.exists():
        raise FileNotFoundError(f"Missing required H5 for neighbors: {h5_path}")

    use_gpu = (not args.no_gpu) and torch.cuda.is_available()
    print(f"GPU enabled: {use_gpu}")

    for latent_label, key1, key2 in LATENT_CONFIGS:
        print(f"\n=== Latents: {latent_label} ({key1}, {key2}) ===")
        emb1, emb2, metadata, param_names = load_h5(h5_path, key1, key2, add_neighbors_derived=True)

        all_results = []
        for objective, task_name, _, _, targets in TASKS:
            print(f"\n--- {objective} :: {task_name} ({len(targets)} targets) ---")
            results_per_variant, task_param_names = run_task(
                task_name,
                targets,
                emb1,
                emb2,
            ...
            all_results.append((objective, task_name, results_per_variant, task_param_names))

        csv_path = output_dir / f"predict_neighbors_ours_debug_{args.suffix}_{latent_label}.csv"
        save_results_csv(all_results, csv_path)

        df = pd.read_csv(csv_path)
        plot_path = output_dir / f"predict_neighbors_ours_debug_{args.suffix}_neighbors_{latent_label}.png"
        plot_objective(df, plot_path, latent_label)
