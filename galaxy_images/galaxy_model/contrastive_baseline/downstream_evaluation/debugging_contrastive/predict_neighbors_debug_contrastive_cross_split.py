"""
Run downstream prediction for neighbors-only objectives using a fixed train/val split.
...
def load_h5_variants(path, key1, key2, add_neighbors_derived=False):
    with h5py.File(path, "r") as f:
        if "split" not in f:
            raise ValueError(f"Missing required split dataset in {path}")
        split_raw = np.array(f["split"][:])
        split_labels = np.array([x.decode("utf-8") if isinstance(x, bytes) else str(x) for x in split_raw])
        ...
        for variant in EMBEDDING_VARIANTS:
            ...
            emb1 = np.array(f[k1][:])
            emb2 = np.array(f[k2][:])
            if emb1.shape[0] != n or emb2.shape[0] != n or len(split_labels) != n:
                raise ValueError(f"Length mismatch in {path}: embeddings vs labels vs split")
            out[variant] = (emb1, emb2, metadata.copy(), list(param_names), split_labels.copy())
    return out
...
def run_task(task_name, h5_stem, default_use_embedding, target_list, h5_path, use_gpu, key1, key2):
    add_derived = h5_stem == "neighbors"
    data = load_h5_variants(h5_path, key1, key2, add_neighbors_derived=add_derived)
    _, _, metadata, param_names, split_labels = data["real"]
    ...
    meta = meta[finite]
    split_labels = split_labels[finite]
    if meta.shape[0] == 0:
        raise ValueError(f"No finite labels for {task_name}")

    tr_idx = np.flatnonzero(split_labels == "train")
    va_idx = np.flatnonzero(split_labels == "val")
    if len(tr_idx) == 0 or len(va_idx) == 0:
        raise ValueError(f"Missing train or val rows in split labels for {task_name}")

    out = {}
    for variant in EMBEDDING_VARIANTS:
        emb1, emb2, _, _, _ = data[variant]
        emb = (
...
def plot_objective(df_obj, objective, out_path, latent_label):
    ...
    ax.set_ylabel("R²")
    ax.set_title(f"Downstream Objective: {objective} ({latent_label})")
...
def main():
    p = argparse.ArgumentParser(description="Predict neighbors-only downstream objectives for the fixed cross-window split.")
    p.add_argument("--suffix", required=True, help="Suffix used in prepare stage")
    p.add_argument("--output-dir", type=Path, default=Path(__file__).resolve().parent / "outputs" / "neighbors_debug_cross_split",
                   help="Directory containing downstream_neighbors_*.h5 and where outputs are written")
    ...
    for latent_label, key1, key2 in LATENT_CONFIGS:
        ...
        csv_path = output_dir / f"predict_neighbors_debug_contrastive_{args.suffix}_{latent_label}.csv"
        save_results_csv(all_results, csv_path)

        df = pd.read_csv(csv_path)
        plot_path = output_dir / f"predict_neighbors_debug_contrastive_{args.suffix}_neighbors_{latent_label}.png"
        plot_objective(df, "neighbors", plot_path, latent_label)
