...
def setup_run_snapshot() -> Path:
    ...
    src_files = [
        script_dir / "train_fm_no-attn.py",
        script_dir / "data.py",
    ]
...
class ConditionalFlowMatchingModule(pl.LightningModule):
    ...
    def __init__(
        ...
        if concat_conditioning:
            ...
            self.velocity_model = UNet2DConditionModel(
                ...
                projection_class_embeddings_input_dim=512, # Matches your ResNet18 output
            )
    ...
    def forward(
        ...
        if self.concat_conditioning:
            ...
            cond_embedding = self.encoder(cond_image)  # (B, 512)
            # Create dummy encoder_hidden_states since it's required but not used without cross-attention
            # Shape: (B, seq_len, cross_attention_dim) - using default cross_attention_dim=1280
            ...
            return self.velocity_model(
                ...
                class_labels=cond_embedding,  # Your ResNet features go here
            ).sample
