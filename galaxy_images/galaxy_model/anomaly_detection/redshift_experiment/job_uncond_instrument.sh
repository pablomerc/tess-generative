...
notify "🅰️ [job $SLURM_JOB_ID] Job A INSTRUMENT (unconditional NSF) starting"
python anomaly_detection/redshift_experiment/fit_score_plot_uncond.py \
    --latents "$LATENTS" --out-dir "$OUTDIR" --top-n 32 --nsf-epochs 50 --webhook "$DISCORD"
if [ $? -ne 0 ]; then notify "❌ [job $SLURM_JOB_ID] Job A instrument FAILED"; exit 1; fi
notify "🎉 [job $SLURM_JOB_ID] Job A instrument done"
