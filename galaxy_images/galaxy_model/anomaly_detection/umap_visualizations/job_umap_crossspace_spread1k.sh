#!/bin/bash
#SBATCH -J umap-crossspace-spread1k
...
SUFFIX="crossspace_spread1k_100k"
...
notify "🚀 [job $SLURM_JOB_ID] umap-crossspace-spread1k starting (30 pts spread over ranks 1-1000)"
mkdir -p "$FIGDIR"
...
if [ $? -ne 0 ]; then
    notify "❌ [job $SLURM_JOB_ID] umap-crossspace-spread1k FAILED"
    exit 1
fi

notify "✅ [job $SLURM_JOB_ID] UMAP done — sending figures..."
notify_file "🗺️ [${SUFFIX}] Physics & AION anomalies (spread ranks 1-1000)" \
    "${FIGDIR}/umap_crossspace_physics_aion_${SUFFIX}.png"
notify_file "🗺️ [${SUFFIX}] Instrument anomalies (spread ranks 1-1000)" \
    "${FIGDIR}/umap_crossspace_instrument_${SUFFIX}.png"

notify "🎉 [job $SLURM_JOB_ID] umap-crossspace-spread1k ALL DONE!"
