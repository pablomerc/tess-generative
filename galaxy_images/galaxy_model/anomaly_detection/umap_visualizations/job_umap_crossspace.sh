...
notify "🚀 [job $SLURM_JOB_ID] umap-crossspace starting (fit UMAP × 3 spaces, project anomalies)"
mkdir -p "$FIGDIR"
...
if [ $? -ne 0 ]; then
    notify "❌ [job $SLURM_JOB_ID] umap-crossspace FAILED"
    exit 1
fi

notify "✅ [job $SLURM_JOB_ID] UMAP done — sending figures..."
notify_file "🗺️ [${SUFFIX}] Physics & AION anomalies across 3 spaces" \
    "${FIGDIR}/umap_crossspace_physics_aion_${SUFFIX}.png"
notify_file "🗺️ [${SUFFIX}] Instrument anomalies across 3 spaces" \
    "${FIGDIR}/umap_crossspace_instrument_${SUFFIX}.png"

notify "🎉 [job $SLURM_JOB_ID] umap-crossspace ALL DONE!"
