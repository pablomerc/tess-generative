...
notify "🚀 [job $SLURM_JOB_ID] pca-tsne-crossspace starting (PCA + t-SNE × 3 spaces × 10k & 100k)"

run_scale() {
    local N_BG=$1
    local SUFFIX="crossspace_${N_BG}"
    local FIGDIR="${OUTDIR}/figures_${SUFFIX}"
    mkdir -p "$FIGDIR"

    notify "▶️ [job $SLURM_JOB_ID] n_background=${N_BG} ..."
    ...
    if [ $? -ne 0 ]; then
        notify "❌ [job $SLURM_JOB_ID] FAILED at n_background=${N_BG}"
        exit 1
    fi

    notify_file "🗺️ [${SUFFIX}] PCA — physics & AION"    "${FIGDIR}/pca_crossspace_physics_aion_${SUFFIX}.png"
    notify_file "🗺️ [${SUFFIX}] PCA — instrument"         "${FIGDIR}/pca_crossspace_instrument_${SUFFIX}.png"
    notify_file "🗺️ [${SUFFIX}] t-SNE — physics & AION"  "${FIGDIR}/tsne_crossspace_physics_aion_${SUFFIX}.png"
    notify_file "🗺️ [${SUFFIX}] t-SNE — instrument"       "${FIGDIR}/tsne_crossspace_instrument_${SUFFIX}.png"

    notify "✅ [job $SLURM_JOB_ID] n_background=${N_BG} done"
}

run_scale 10000
run_scale 100000

notify "🎉 [job $SLURM_JOB_ID] pca-tsne-crossspace ALL DONE!"
