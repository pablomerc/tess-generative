"""
Encode all source_type=0 (MMU) examples using encoder_2 (instrument latent).
Saves three latent representations of encoder_2 per example, analogous to
encode_latents_ours.py which uses encoder_1 (physics latent).
...
def encode(checkpoint_path, suffix, batch_size, n_max, device_str):
    ...
    ins_dim = model.instrument_zdim  # already resolved in __init__: None → cross_attention_dim
    ...
    with torch.no_grad():
        for hsc_imgs, leg_imgs, raw_indices in tqdm(loader, desc="Encoding"):
            ...
            enc2_hsc = model.encoder_2(hsc_imgs)   # [B, n_tokens, ins_dim]
            enc2_leg = model.encoder_2(leg_imgs)   # [B, n_tokens, ins_dim]

            hsc_flat = enc2_hsc.flatten(start_dim=1).cpu().numpy().astype(np.float32)
            hsc_mean = enc2_hsc.mean(dim=1).cpu().numpy().astype(np.float32)
            hsc_legacy_mean = torch.cat([enc2_hsc, enc2_leg], dim=1).mean(dim=1).cpu().numpy().astype(np.float32)
    ...
    print(f"  hsc_flat:  {hsc_flat.shape}  (n_tokens={enc2_hsc.shape[1]}, ins_dim={ins_dim})")
    ...
    tmp_fd, tmp_path = tempfile.mkstemp(suffix=".h5", prefix="anomaly_latents_ins_", dir=tempfile.gettempdir())
    os.close(tmp_fd)
    try:
        with h5py.File(tmp_path, "w") as f:
            ...
            f.attrs["n_tokens"] = int(enc2_hsc.shape[1])
