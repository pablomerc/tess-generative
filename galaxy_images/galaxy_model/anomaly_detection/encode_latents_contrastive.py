"""
Encode all source_type=0 (MMU) examples using the trained dual-encoder contrastive model.

Saves three latent representations per example (keyed to match fit_and_score.py):
  hsc_flat        - encoder_galaxy(hsc_img)                      [N, embedding_dim]
  hsc_mean        - encoder_galaxy(hsc_img)  (same as flat)      [N, embedding_dim]
  hsc_legacy_mean - mean(encoder_galaxy(hsc), encoder_galaxy(legacy)) [N, embedding_dim]
  instrument_emb  - encoder_instrument(hsc_img)                  [N, embedding_dim]
...
def encode(checkpoint_path, suffix, batch_size, n_max, device_str):
    ...
    with torch.no_grad():
        for hsc_imgs, leg_imgs, raw_indices in tqdm(loader, desc="Encoding"):
            ...
            z_galaxy_hsc = model.encoder_galaxy(hsc_imgs)     # [B, embedding_dim]
            z_galaxy_leg = model.encoder_galaxy(leg_imgs)     # [B, embedding_dim]
            z_instrument = model.encoder_instrument(hsc_imgs) # [B, embedding_dim]

            hsc_mean = z_galaxy_hsc.cpu().numpy().astype(np.float32)
            hsc_legacy_mean = ((z_galaxy_hsc + z_galaxy_leg) / 2).cpu().numpy().astype(np.float32)
    ...
    tmp_fd, tmp_path = tempfile.mkstemp(
        suffix=".h5", prefix="anomaly_latents_contrastive_", dir=tempfile.gettempdir()
    )
    os.close(tmp_fd)
    try:
        with h5py.File(tmp_path, "w") as f:
            f.create_dataset("raw_index",        data=raw_index,        compression="gzip", compression_opts=4)
            # hsc_flat == hsc_mean for single-vector encoders (no token dimension)
