#!/bin/bash
#SBATCH -J snr-pct-hist-128-1024
...
notify "📈 [job $SLURM_JOB_ID] SNR-percentile histograms (top-128 + top-1024) starting..."

for TOP_N in 128 1024; do
    for METHOD in flow iforest; do
        ...
        if [ $? -ne 0 ]; then
            notify "❌ [job $SLURM_JOB_ID] top-${TOP_N} ${METHOD} FAILED"
            exit 1
        fi
...
notify "✅ [job $SLURM_JOB_ID] SNR-percentile histograms (128 + 1024) DONE"
