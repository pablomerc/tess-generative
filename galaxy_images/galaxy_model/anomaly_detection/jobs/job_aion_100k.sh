...
notify "🚀 [job $SLURM_JOB_ID] anomaly-aion-100k starting (wall time: 4 days). encode → fit → visualize"

# ── Step 1: Encode ──────────────────────────────────────────────────────────
notify "🔢 [job $SLURM_JOB_ID] Step 1/3: encoding all ~100k MMU examples with AION (batch_size=32)..."
python anomaly_detection/encode_latents_aion.py \
    --suffix "$SUFFIX" \
    --batch-size 32
if [ $? -ne 0 ]; then notify "❌ [job $SLURM_JOB_ID] AION encode FAILED"; exit 1; fi
notify "✅ [job $SLURM_JOB_ID] Step 1/3 done: AION latents saved (~100k)"
...
notify "🎉 [job $SLURM_JOB_ID] anomaly-aion-100k ALL DONE!"
