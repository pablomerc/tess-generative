...
if [ -z "$CKPT" ]; then
    notify "❌ [job $SLURM_JOB_ID] anomaly-cnt-100k: no best checkpoint found in ${CKPT_SEARCH_DIR}"
    exit 1
fi
notify "🚀 [job $SLURM_JOB_ID] anomaly-cnt-100k starting with checkpoint: $(basename $CKPT)"

# ── Step 1: Encode ──────────────────────────────────────────────────────────
notify "🔢 [job $SLURM_JOB_ID] Step 1/3: encoding all ~100k MMU examples with contrastive model..."
...
notify "🎉 [job $SLURM_JOB_ID] anomaly-cnt-100k ALL DONE!"
