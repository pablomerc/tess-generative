"""Encode HSC downstream dataset with the trained dual-encoder's INSTRUMENT encoder.

Mirrors anomaly_detection/encode_latents_instrument.py but on the HSC downstream
memmap+parquet dataset (no Legacy branch -> no `hsc_legacy_mean`).

Output suffix prefix is `ins_` to match the original convention used by the
paper figure code.

Run:
  python anomaly_detection/engaging/encode_instrument_hscdown.py \
...
def encode(checkpoint_path, suffix, batch_size, n_max, device_str):
    ...
    instrument_zdim = getattr(model.hparams, "instrument_zdim", None)
    print(f"  instrument_zdim={instrument_zdim}")
    ...
    written = 0
    with torch.no_grad():
        for imgs, raw in tqdm(loader, desc="Encoding (ours-instrument)"):
            imgs = imgs.to(device, non_blocking=True)
            enc = model.encoder_2(imgs)  # (B, n_tokens, instrument_zdim)
    ...
    print(f"  hsc_flat:  {hsc_flat.shape}  (n_tokens={n_tokens})")
    print(f"  hsc_mean:  {hsc_mean.shape}")

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    out_path = OUTPUT_DIR / f"anomaly_latents_ins_{suffix}.h5"
    tmp_fd, tmp_path = tempfile.mkstemp(
        suffix=".h5", prefix="anomaly_latents_ins_hscdown_", dir=tempfile.gettempdir()
    )
    os.close(tmp_fd)
    try:
        with h5py.File(tmp_path, "w") as f:
            ...
            if instrument_zdim is not None:
                f.attrs["instrument_zdim"] = int(instrument_zdim)
