#!/bin/bash
#SBATCH -J anom-hscdown-ins
#SBATCH -o /home/pablomer/orcd/pool/tess-generative/galaxy_images/galaxy_model/anomaly_detection/engaging/logs/ins.%j.out
#SBATCH -e /home/pablomer/orcd/pool/tess-generative/galaxy_images/galaxy_model/anomaly_detection/engaging/logs/ins.%j.err
...
LATENTS="${OUTDIR}/anomaly_latents_ins_${SUFFIX}.h5"
SCORES="${OUTDIR}/anomaly_scores_ins_${SUFFIX}.h5"
...
notify "🚀 [job $SLURM_JOB_ID] hscdown-ours-instrument start | host=$(hostname -s) | GPU: ${GPU_INFO}"

# ── Step 1: Encode ─────────────────────────────────────────────────────────────
notify "🔢 [job $SLURM_JOB_ID] (1/3) encode_instrument_hscdown.py — 474,954 examples"
...
for METHOD in flow iforest; do
    OUT="${FIGDIR}/top12_ins_${METHOD}_${SUFFIX}.png"
    python anomaly_detection/engaging/plot_top12_hscdown.py \
        --scores "$SCORES" \
        --score-key "ours/hsc_flat/${METHOD}" \
        --label "Ours (Instrument) — ${METHOD} — hsc_flat" \
        --out "$OUT"
    if [ $? -ne 0 ]; then notify "⚠️ [job $SLURM_JOB_ID] plot ${METHOD} FAILED"; continue; fi
    notify_file "📊 [hscdown ours-instrument ${METHOD}] top-12" "$OUT"
done

notify "🎉 [job $SLURM_JOB_ID] hscdown-ours-instrument ALL DONE"
